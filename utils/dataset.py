...
class OrhogonalPhotoDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if (
            ...
            raise AssertionError(f"Either no mask or multiple masks found for the ID {idx}: {msk_file}")
        if (
            len(img_file) != 1
        ):
            raise AssertionError(f"Either no image or multiple images found for the ID {idx}: {img_file}")
        ...
        if (
            ...
            raise AssertionError(f"Image and mask {idx} should be the same size, but are {img_as_img.size} and {msk_as_img.size}")
